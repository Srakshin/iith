...
from credit.routes import router as credit_router
...
try:
...
except Exception as exc:  # pragma: no cover - fallback for local Phase 1 shell
    ...
    def update_environment_variables(*args, **kwargs):
        return None

    async def handle_file_upload(*args, **kwargs):
        raise HTTPException(
            status_code=503,
            detail=f"Legacy upload flow is unavailable in this local setup: {RESEARCH_IMPORT_ERROR}",
        )

    async def handle_file_deletion(*args, **kwargs):
        raise HTTPException(
            status_code=503,
            detail=f"Legacy file deletion is unavailable in this local setup: {RESEARCH_IMPORT_ERROR}",
        )

    async def execute_multi_agents(*args, **kwargs):
        raise HTTPException(
            status_code=503,
            detail=f"Multi-agent research is unavailable in this local setup: {RESEARCH_IMPORT_ERROR}",
        )

    async def handle_websocket_communication(*args, **kwargs):
        raise RuntimeError(
            f"WebSocket research flow is unavailable in this local setup: {RESEARCH_IMPORT_ERROR}"
        )

try:
    from server.websocket_manager import WebSocketManager, run_agent
...
# WebSocket manager
manager = WebSocketManager() if WebSocketManager is not None else None
...
async def write_report(research_request: ResearchRequest, research_id: str = None):
    if run_agent is None or Tone is None:
        raise HTTPException(
            status_code=503,
            detail=f"Research stack is unavailable in this local setup: {RESEARCH_IMPORT_ERROR}",
        )
...
async def websocket_endpoint(websocket: WebSocket):
    if manager is None:
        await websocket.close(code=1011, reason="Research stack unavailable")
        return
...
async def chat(chat_request: ChatRequest):
    ...
    try:
        if ChatAgentWithMemory is None:
            raise HTTPException(
                status_code=503,
                detail=f"Chat stack is unavailable in this local setup: {RESEARCH_IMPORT_ERROR}",
            )
