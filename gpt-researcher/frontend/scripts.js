const GPTResearcher = (() => {
  ...
  const init = () => {
    ...
    // Initialize the Intelli-Credit CAM demo panel
    initCreditCamDemo();
...
- Click Cancel to REPLACE all existing history with imported data`)) {
  ...
  const initCreditCamDemo = () => {
    const createBtn = document.getElementById('creditCreateCaseBtn');
    const demoBtn = document.getElementById('creditRunDemoBtn');
    const workflowBtn = document.getElementById('creditRunWorkflowBtn');
    const camBtn = document.getElementById('creditGenerateCamBtn');

    if (!createBtn || !demoBtn || !workflowBtn || !camBtn) {
      return;
    }

    createBtn.addEventListener('click', createCreditCaseAndUpload);
    demoBtn.addEventListener('click', runSyntheticCreditDemo);
    workflowBtn.addEventListener('click', runCreditWorkflow);
    camBtn.addEventListener('click', generateCreditCamOnly);
    updateCreditPanel();
  };

  const creditFetch = async (path, options = {}) => {
  ...
  const setCreditBusy = (busy) => {
    creditBusy = busy;
    ['creditCreateCaseBtn', 'creditRunDemoBtn', 'creditRunWorkflowBtn', 'creditGenerateCamBtn'].forEach((id) => {
      const element = document.getElementById(id);
      if (element) {
  ...
  const updateCreditStatus = (message, isError = false) => {
    const statusEl = document.getElementById('creditCaseStatus');
    if (!statusEl) return;
    statusEl.textContent = message;
    statusEl.style.color = isError ? '#fca5a5' : 'rgba(191, 219, 254, 0.88)';
  };

  const creditInputValue = (id) => {
    const element = document.getElementById(id);
    return element ? element.value.trim() : '';
  };

  const collectCreditNotesPayload = () => {
    const payload = {
      management_quality: creditInputValue('creditManagementQuality'),
      governance_concerns: creditInputValue('creditGovernanceConcerns'),
      factory_operating_capacity: creditInputValue('creditOperatingCapacity'),
      site_visit_comments: creditInputValue('creditSiteVisitComments'),
      collateral_observations: creditInputValue('creditCollateralObservations'),
      additional_comments: creditInputValue('creditAdditionalComments'),
      updated_by: 'credit-cam-ui'
    };

    const hasAnyNotes = Object.entries(payload).some(([key, value]) => key !== 'updated_by' && value);
    return hasAnyNotes ? payload : null;
  };

  const populateCreditNoteInputs = (casePayload) => {
    const notes = casePayload?.dossier?.qualitative_credit_officer_notes || {};
    const mapping = {
      creditManagementQuality: notes.management_quality || '',
      creditGovernanceConcerns: notes.governance_concerns || '',
      creditOperatingCapacity: notes.factory_operating_capacity || '',
      creditSiteVisitComments: notes.site_visit_comments || '',
      creditCollateralObservations: notes.collateral_observations || '',
      creditAdditionalComments: notes.additional_comments || ''
    };
  ...
  const applyCreditResponse = (casePayload, camPayload = null) => {
    creditCase = casePayload || creditCase;
    creditCaseId = creditCase?.case_id || creditCaseId;
    creditCam = camPayload || creditCase?.dossier?.credit_appraisal_memo || creditCam;
    if (creditCase) {
      populateCreditNoteInputs(creditCase);
    }
    updateCreditPanel();
  };

  const createCreditCaseAndUpload = async () => {
    if (creditBusy) return null;

    const borrowerName = creditInputValue('creditBorrowerName');
    const externalReference = creditInputValue('creditExternalReference');
    const fileInput = document.getElementById('creditFiles');
    const files = fileInput?.files ? Array.from(fileInput.files) : [];

    if (!borrowerName) {
      showToast('Enter a borrower name before creating a credit case.');
      return null;
    }

    setCreditBusy(true);
    updateCreditStatus('Creating credit case...');

    try {
      const createPayload = await creditFetch('/api/credit/cases', {
        method: 'POST',
        body: JSON.stringify({
      ...
      applyCreditResponse(createPayload.case, null);
      updateCreditStatus(`Created case ${createPayload.case.case_id}.`);

      if (files.length > 0) {
        const formData = new FormData();
        files.forEach((file) => formData.append('files', file));
        updateCreditStatus(`Uploading ${files.length} borrower file${files.length === 1 ? '' : 's'}...`);
        const uploadPayload = await creditFetch(`/api/credit/cases/${createPayload.case.case_id}/files`, {
          method: 'POST',
          body: formData
        });
        applyCreditResponse(uploadPayload.case, null);
        updateCreditStatus(`Uploaded ${files.length} file${files.length === 1 ? '' : 's'} to ${uploadPayload.case.case_id}.`);
      } else {
        showToast('Case created without borrower files. You can still upload files later or run the synthetic demo.');
      }

      return creditCaseId;
    } catch (error) {
      console.error('Error creating credit case:', error);
      updateCreditStatus(`Credit case setup failed: ${error.message}`, true);
      showToast(`Credit case setup failed: ${error.message}`);
      return null;
    } finally {
      setCreditBusy(false);
    }
  ...
  const runCreditWorkflow = async () => {
    if (creditBusy) return;

    let activeCaseId = creditCaseId;
    if (!activeCaseId) {
      activeCaseId = await createCreditCaseAndUpload();
    }

    if (!activeCaseId || !creditCase?.uploaded_files?.length) {
      showToast('Upload borrower files or run the synthetic demo before launching the CAM workflow.');
      return;
    }

    setCreditBusy(true);
    try {
      updateCreditStatus('Ingesting borrower documents...');
      const ingestPayload = await creditFetch(`/api/credit/cases/${activeCaseId}/ingest`, { method: 'POST' });
      applyCreditResponse(ingestPayload.case, null);

      updateCreditStatus('Computing credit features...');
      await creditFetch(`/api/credit/cases/${activeCaseId}/features`, { method: 'POST' });

      const notesPayload = collectCreditNotesPayload();
      if (notesPayload) {
        updateCreditStatus('Saving primary due-diligence notes...');
        const notesResponse = await creditFetch(`/api/credit/cases/${activeCaseId}/notes`, {
          method: 'POST',
          body: JSON.stringify(notesPayload)
        });
        applyCreditResponse(notesResponse.case, null);
      }

      updateCreditStatus('Running secondary research...');
      const researchPayload = await creditFetch(`/api/credit/cases/${activeCaseId}/research`, {
        method: 'POST',
        body: JSON.stringify({})
      });
      applyCreditResponse(researchPayload.case, null);

      updateCreditStatus('Generating the explainable recommendation...');
      const scorePayload = await creditFetch(`/api/credit/cases/${activeCaseId}/score`, { method: 'POST' });
      applyCreditResponse(scorePayload.case, null);

      updateCreditStatus('Generating CAM markdown, PDF, and DOCX exports...');
      const camPayload = await creditFetch(`/api/credit/cases/${activeCaseId}/cam`, { method: 'POST' });
      applyCreditResponse(camPayload.case, camPayload.cam);

      updateCreditStatus(`CAM ready for ${activeCaseId}.`);
      showToast('Credit Appraisal Memo generated successfully.');
    } catch (error) {
      console.error('Error running CAM workflow:', error);
      updateCreditStatus(`CAM workflow failed: ${error.message}`, true);
      showToast(`CAM workflow failed: ${error.message}`);
    } finally {
      setCreditBusy(false);
    }
  };

  const generateCreditCamOnly = async () => {
    if (creditBusy || !creditCaseId) {
      showToast('Create a credit case first or run the synthetic demo.');
      return;
    }

    setCreditBusy(true);
    updateCreditStatus('Generating CAM for the current case...');
    try {
      const camPayload = await creditFetch(`/api/credit/cases/${creditCaseId}/cam`, { method: 'POST' });
      applyCreditResponse(camPayload.case, camPayload.cam);
      updateCreditStatus(`CAM ready for ${creditCaseId}.`);
      showToast('CAM regenerated for the current case.');
    } catch (error) {
      console.error('Error generating CAM:', error);
      updateCreditStatus(`CAM generation failed: ${error.message}`, true);
      showToast(`CAM generation failed: ${error.message}`);
    } finally {
      setCreditBusy(false);
    }
  };

  const runSyntheticCreditDemo = async () => {
    if (creditBusy) return;

    setCreditBusy(true);
    updateCreditStatus('Running the bundled synthetic CAM demo...');
    try {
      const demoPayload = await creditFetch('/api/credit/demo/run', { method: 'POST' });
      applyCreditResponse(demoPayload.case, demoPayload.cam);
      updateCreditStatus(`Synthetic demo completed for ${demoPayload.case.case_id}.`);
      showToast('Synthetic Intelli-Credit demo completed.');
    } catch (error) {
      console.error('Error running synthetic CAM demo:', error);
      updateCreditStatus(`Synthetic demo failed: ${error.message}`, true);
      showToast(`Synthetic demo failed: ${error.message}`);
    } finally {
      setCreditBusy(false);
    }
  };

  const updateCreditPanel = () => {
    const metaEl = document.getElementById('creditCaseMeta');
    const summaryEl = document.getElementById('creditCamSummary');
    const evidenceEl = document.getElementById('creditCamEvidence');

    if (!metaEl || !summaryEl || !evidenceEl) {
      return;
    }

    if (!creditCase && !creditCam) {
      metaEl.textContent = 'Create a case or run the synthetic demo to populate this panel.';
      summaryEl.innerHTML = '<p class="credit-placeholder">No credit appraisal memo generated yet.</p>';
      evidenceEl.innerHTML = '<p class="credit-placeholder">Evidence-backed CAM highlights will appear here after generation.</p>';
      updateCreditDownloadLinks();
      return;
    }

    const uploadedFiles = creditCase?.uploaded_files?.length || 0;
    metaEl.textContent = `Case ${creditCase?.case_id || 'n/a'} | status ${creditCase?.status || 'unknown'} | ${uploadedFiles} uploaded file${uploadedFiles === 1 ? '' : 's'}.`;

    if (!creditCam) {
      summaryEl.innerHTML = '<p class="credit-placeholder">Case available. Generate the CAM to populate the memo summary and exports.</p>';
      evidenceEl.innerHTML = '<p class="credit-placeholder">Generate the CAM to surface evidence highlights.</p>';
      updateCreditDownloadLinks();
      return;
    }

    summaryEl.innerHTML = `
      <div class="credit-summary-metrics">
        <div class="credit-metric-card">
          <span class="credit-metric-label">Decision</span>
          <span class="credit-metric-value">${escapeHtml((creditCam.decision || '').toUpperCase())}</span>
        </div>
        <div class="credit-metric-card">
          <span class="credit-metric-label">Risk Score</span>
          <span class="credit-metric-value">${escapeHtml(Number(creditCam.overall_risk_score || 0).toFixed(1))}</span>
        </div>
        <div class="credit-metric-card">
          <span class="credit-metric-label">Recommended Limit</span>
          <span class="credit-metric-value">${escapeHtml(formatCreditAmount(creditCam.recommended_limit))}</span>
        </div>
        <div class="credit-metric-card">
          <span class="credit-metric-label">Risk Premium</span>
          <span class="credit-metric-value">${escapeHtml(`${creditCam.pricing?.risk_premium_bps || 0} bps`)}</span>
        </div>
      </div>
      ${renderCreditSectionSnippet('Final Recommendation', creditCam.final_recommendation)}
      ${renderCreditSectionSnippet('Decision Rationale', creditCam.decision_rationale)}
      ${renderCreditSectionSnippet('Research Findings', creditCam.research_findings_and_flags)}
    `;

    const evidenceItems = [
      ...(creditCam.decision_rationale?.evidence || []),
      ...(creditCam.research_findings_and_flags?.evidence || []),
      ...(creditCam.primary_due_diligence_notes?.evidence || [])
    ].slice(0, 8);

    evidenceEl.innerHTML = evidenceItems.length
      ? evidenceItems.map(renderCreditEvidenceItem).join('')
      : '<p class="credit-placeholder">No evidence highlights were attached to this memo.</p>';

    updateCreditDownloadLinks();
  };

  const renderCreditSectionSnippet = (title, section) => {
    if (!section) return '';
    ...
    return `
      <div class="credit-summary-section">
        ...
        <p>${escapeHtml(section.summary || 'No summary available.')}</p>
        ${bullets ? `<ul>${bullets}</ul>` : ''}
      </div>
    `;
  };

  const renderCreditEvidenceItem = (item) => {
    const parts = [];
    if (item.reference) {
      parts.push(`<span>${escapeHtml(item.reference)}</span>`);
    }
    if (item.source_path) {
      parts.push(`<span>Path: <code>${escapeHtml(item.source_path)}</code></span>`);
    }
    if (item.source_url) {
      parts.push(`<a href="${item.source_url}" target="_blank" rel="noopener noreferrer">Source link</a>`);
    }

    return `
      <div class="credit-evidence-item">
  ...
  const formatCreditAmount = (amountPayload) => {
    if (!amountPayload) return 'Not available';
    const amount = typeof amountPayload.amount === 'number'
      ? amountPayload.amount
      : Number(amountPayload.amount || 0);
    const currency = amountPayload.currency || 'INR';
    if (!Number.isFinite(amount)) return `${currency} n/a`;
    return `${currency} ${amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  };

  const updateCreditDownloadLinks = () => {
    const mapping = {
      creditCamDownloadMd: 'md',
      creditCamDownloadPdf: 'pdf',
      creditCamDownloadDocx: 'docx'
    };

    Object.entries(mapping).forEach(([id, format]) => {
      const anchor = document.getElementById(id);
      if (!anchor) return;

      if (creditCaseId && creditCam) {
        anchor.href = `/api/credit/cases/${creditCaseId}/cam/download/${format}`;
